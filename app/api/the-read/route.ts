...
export const POST = withLog("the-read", async (req: Request) => {
  ...
  const userMessage = trimmedTaskPrompt
  ...
  if (!upstream.ok) {
    return NextResponse.json(
      { error: `OpenRouter ${upstream.status}`, raw },
      { status: 502 },
    );
  }
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Non-JSON response from OpenRouter", raw },
      { status: 502 },
    );
  }
